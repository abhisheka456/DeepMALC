...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_best_weights', default='final_weights')
    parser.add_argument('--best_acc', default=0.00, type=float)
...
origin = 'weights'
target = args.save_best_weights
file_name = '/'+ args.dataset+ '_embedd.zip' 
...
for i in range(100):
    ...
    f.close()
    if args.best_acc < t_acc:
        args.best_acc = t_acc
        shutil.copy(origin+file_name, target+file_name)
        np.savetxt(args.save_dir + "/" + args.dataset + '-clusters.txt', clusters, fmt='%i', delimiter=',')
        print('weights is saved in Final folder')
    else:
        shutil.copy(target+file_name, origin+file_name)
